...
class MotorPIDStats:
    ...
    def avg_convergence_time(self) -> float:
        return (self.total_convergence_time / self.successful_runs) if self.successful_runs > 0 else 0.0
...
class PIDAnalyzer:
    ...
    def update_pid_status(self, motor: str, current_angle: float, error: float, 
    ...
    def finish_pid_move(self, motor: str, status: PIDStatus, 
                        final_angle: float, final_error: float) -> Optional[PIDRunRecord]:
        """完成 PID 运动"""
        if motor not in self.active_records:
            return None
    ...
    def get_stats_summary(self, motor: str) -> Dict:
        ...
        return {
            ...
            "min_convergence_time": f"{stats.min_convergence_time:.2f}s" if stats.min_convergence_time else "N/A",
            "max_convergence_time": f"{stats.max_convergence_time:.2f}s" if stats.max_convergence_time else "N/A",
