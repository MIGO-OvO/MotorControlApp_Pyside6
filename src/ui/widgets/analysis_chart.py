...
from PySide6.QtCharts import QChart, QLineSeries, QScatterSeries, QValueAxis
...
class AnalysisChart(QChart):
    ...
    def update_data(self, deviations):
        ...
        try:
            ...
            parent = self.parent()
            active_motors = (
                parent.active_motors if hasattr(parent, "active_motors") else ["X", "Y", "Z", "A"]
            )
    ...
    def get_chart_data(self):
        """获取当前图表数据"""
        return {motor: list(self.data[motor]) for motor in ["X", "Y", "Z", "A"]}
